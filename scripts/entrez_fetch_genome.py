'''
This script is designed to take a list of NC_ IDs from NCBI and download the genome for them from NCBI via entrez.
...
def request_cds( org_id, email ):
    Entrez.email = email
    handle = Entrez.efetch(db="nuccore", id=org_id, rettype="fasta", retmode="text")
    text = handle.read()
    return text
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
            description = "Utilize entrez to download genomes from NCBI"
            )
