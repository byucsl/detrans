...
n_hidden = lstm_size # hidden layer num of features
...
weights = {
    ...
    'out': tf.Variable(tf.random_normal( [  ( lstm_size * 2 ) * number_of_layers, n_classes ] ) )
}
...
outputs = rnn.bidirectional_rnn(
        ...
        _x,
        #unpack_sequence( _x ),
        #tf.unpack( tf.transpose( _x, perm = [ 1, 0, 2 ] ) ),
...
#packed_states = pack_sequence( states )
